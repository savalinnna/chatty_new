...
async def subscribe_to_user(
    ...
    existing_subscription = await db.execute(
        select(Subscription).where(
            ...
            Subscription.follower_id == current_user
        )
    )
    if existing_subscription.scalars().first():
        raise HTTPException(status_code=400, detail="Already subscribed")

    subscription = Subscription(user_id=user_id, follower_id=current_user)
    db.add(subscription)
    await db.commit()
...
async def list_subscriptions(
    ...
    result = await db.execute(
        select(Subscription).where(Subscription.follower_id == current_user)
    )
...
async def get_following_users(
    ...
    result = await db.execute(
        select(Subscription.user_id).where(Subscription.follower_id == current_user)
    )
    return result.scalars().all()
...
async def get_followers(
    ...
    result = await db.execute(
        select(Subscription.follower_id).where(Subscription.user_id == current_user)
    )
    return result.scalars().all()
